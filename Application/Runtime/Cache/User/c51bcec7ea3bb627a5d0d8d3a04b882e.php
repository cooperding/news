...
<div class="row">
    ...
    <div class="col-md-9">
        <div class="dogo-member">
            ...
            <form class="form-horizontal " role="form" action="<?php echo U('User/Index/apiUpdate');?>" method="post">
                <input name="id" value="<?php echo ($data["id"]); ?>" type="hidden"/>
                <div class="form-group">
                    <label for="inputApiToken" class="col-sm-2 control-label">API Token：</label>
                    <div class="col-sm-10">
                        <input type="text" name="apitoken" class="form-control input-lg" id="inputApiToken" placeholder="请填写token信息(例：adminsir)" value="<?php echo ($data["apitoken"]); ?>" required="required">
                    </div>
                </div><!--form-group-->
                <div class="form-group">
                    <label for="inputApiSecretkey" class="col-sm-2 control-label">API密钥：</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control input-lg" id="inputApiSecretkey" disabled placeholder="API密钥" value="<?php echo ($data["secretkey"]); ?>" required="required">
                    </div>
                </div><!--form-group-->
                
                <div class="form-group">
                    <label for="inputSignature" class="col-sm-2 control-label">API签名：</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control input-lg" id="inputSignature" disabled placeholder="API签名" value="<?php echo ($data["signature"]); ?>" required="required">
                    </div>
                </div><!--form-group-->
                <div class="form-group">
                    <label for="inputSex" class="col-sm-2 control-label">API状态：</label>
                    <div class="col-sm-10">
                        <label class="radio-inline">
                            <?php if($data["status"] == '20'): ?>可用
                                <?php else: ?>
                                禁用<?php endif; ?>
                        </label>
                    </div>
                ...
                <div class="form-group">
                    <label for="inputDomain" class="col-sm-2 control-label">站点域名：</label>
                    <div class="col-sm-10">
                        <textarea class="form-control input-lg" name="domain" id="inputDomain" rows="3" required="required" 
                                  placeholder="站点域名，多个域名使用“|”分开,域名示例adminsir.net|www.adminsir.net"><?php echo ($data["domain"]); ?></textarea>
                        （注：如果内容有更新，需等管理员重新审核通过。）
                    </div>
                </div><!--form-group-->
                <div class="form-group">
                    <div class=" col-sm-12">
