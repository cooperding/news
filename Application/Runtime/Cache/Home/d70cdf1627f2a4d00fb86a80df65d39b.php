    ...
    <div class="col-md-8">
        ...
        <article class="dogo-box dogo-border-all dogo-pd515">
            <div class="dogo-box-header">
                <h5><span class="glyphicon glyphicon-hand-right"></span> <?php echo ($data["title"]); ?></h5>
                <p>
                    <span class="glyphicon glyphicon-user"></span> <a href="<?php echo U('Home/User/index',array('uid'=>$data['members_id']));?>"><?php echo ($data["username"]); ?> </a>
                    <span class="glyphicon glyphicon-time"></span>  <?php echo (date("Y-m-d H:i:s",$data["addtime"])); ?>
                </p>
            </div>
            <div class="dogo-box-content dogo-view">
                <p><?php if($data["titlepic"] != ''): ?><br/>
                    <img class="img-responsive" src="<?php echo ($data["titlepic"]); ?>"/><?php endif; ?>
            ...
            <div class="dogo-box-footer">
                <div class="row">
                    <div class="col-md-6">
                        <span type="button" class="btn btn-default dogo-click-up disabled_<?php echo ($data["id"]); ?>" data-row-id="<?php echo ($data["id"]); ?>" title="顶"> <span class="glyphicon glyphicon-thumbs-up"></span> <span class="num_up_<?php echo ($data["id"]); ?>"><?php echo ($data["num_top"]); ?></span></span>
                        <span type="button" class="btn btn-default dogo-click-down disabled_<?php echo ($data["id"]); ?>" data-row-id="<?php echo ($data["id"]); ?>" title="踩"> <span class="glyphicon glyphicon-thumbs-down"></span> <span class="num_down_<?php echo ($data["id"]); ?>"><?php echo ($data["num_beat"]); ?></span></span>
                    </div>
                    <div class="col-md-6 dogo-align">
                        <span type="button" class="btn btn-default" title="评论"> <span class="glyphicon glyphicon-comment"></span> <span class="num_comment_<?php echo ($data["id"]); ?>"><?php echo ($data["num_comment"]); ?></span></span>
        ...
        <section class="dogo-box dogo-border-all dogo-pd515">
            ...
            <div class="dogo-box-content dogo-list-comment">
                <ul>
                    <?php if(is_array($list_comment)): foreach($list_comment as $key=>$vo): ?><li><span class="glyphicon glyphicon-user"></span> 
                            <a href="<?php echo U('Home/User/index',array('uid'=>$vo['members_id']));?>"><?php echo ($vo["username"]); ?> </a>
                            <?php echo ($vo["content"]); ?>
                            <span class="dogo-frt">#<?php echo ($vo["floor"]); ?></span>
            ...
            <div class="dogo-box-content">
                <div class="dogo-text">
                    ...
                    <ul class="dogo-text-more">
                        <?php $qiuyun = new \Org\Util\Qiuyun; $_result=$qiuyun->list_to_tree(M('NewsSort')->where(" (`status`='20') ")->select(),"id", "parent_id", "children"); if ($_result): $i=0;foreach($_result as $key=>$sort):++$i;$mod = ($i % 2 );?><li><a href="#"><?php echo ($sort["text"]); ?></a></li>
                            <?php if(is_array($sort["children"])): $i = 0; $__LIST__ = $sort["children"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><a href="#"><?php echo ($vo["text"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; endforeach; endif;?>
                    </ul>
                </div><!--dogo-text-->
            </div><!--dogo-box-content-->
            <div class="dogo-box-footer">
            ...
            <div class="dogo-box-content">
                <div class="dogo-text">
                    ...
                    <ul class="dogo-text-onelines">
                        <?php $_result=M('Title')->table('ding_title t')->join(' ding_news_sort ns on ns.id=t.sort_id ')->join(' ding_content c on c.title_id=t.id ')->join(' ding_members m on m.id=t.members_id ')->field( 't.*,c.content,m.username,ns.text as sortname' )->order('t.id desc')->where(" (t.`status`='12')  and (t.`is_recycle`='10') ")->limit(10)->select(); if ($_result): $i=0;foreach($_result as $key=>$article):++$i;$mod = ($i % 2 );?><li><a href="<?php echo U('Home/Content/index',array('id'=>$article['id']));?>"><?php echo ($article["title"]); ?></a></li><?php endforeach; endif;?>
                    </ul>
                </div>
            </div><!--dogo-box-content-->
            <div class="dogo-box-footer">
