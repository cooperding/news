<?php if (!defined('THINK_PATH')) exit();?><table  id="datagrid_goodslisttab<?php echo ($id); ?>" class="datagrid">
...
<script>
    ...
    $(function() {
        ...
        var classId = 'goodslisttab<?php echo ($id); ?>';
        var hrefadd = '<?php echo U("Admin/GoodsList/add",array("id"=>$id));?>';
        var hrefedit = '<?php echo U("Admin/GoodsList/edit");?>';
        var hrefcancel = '<?php echo U("Admin/GoodsList/delete");?>';
        var urljson = '<?php echo U("Admin/GoodsList/listJsonId",array("id"=>$id));?>';
        var hrefgallery = '<?php echo U("Admin/GoodsList/gallery");?>';
            ...
            columns: [[
                    ...
                    //{field:'parent_id',title:'parent_id',width:200},
                    {
                        ...
                        formatter: function(value, row, index) {
                            return '<img class="btn_do" src="/qiuyun/dogocms32/Public/Easyui/themes/icons/tip.png" onclick="ding_gallery(\'' + hrefgallery + '?id=' + row.id + '\',\'' + classId + '\')"  title="商品图片集"/>&nbsp;\n\
                    <img class="btn_do" src="/qiuyun/dogocms32/Public/Easyui/themes/icons/pencil.png" onclick="ding_edit(\'' + hrefedit + '?id=' + row.id + '\',\'' + classId + '\')"  title="编辑"/>&nbsp;\n\
<img class="btn_do" src="/qiuyun/dogocms32/Public/Easyui/themes/icons/cancel.png" onclick="ding_cancel(\'' + row.id + '\',\'' + hrefcancel + '\',\'' + classId + '\')" title=" 删除"/>&nbsp;';
            ...
            toolbar: [{
                    ...
                    text: '<input class="ajax_goodslist" placeholder="商品名称"></input> '
                }, {
                    id: 'btnsubmit' + classId,
                    //text: '搜索',
                    iconCls: 'icon-search',
                    handler: function() {
                        var title = $('.ajax_goodslist').val();
